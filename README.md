# Pinuyumayan Backend Database

這不是技能包。這是 **卑南族文化綜合平台後端資料庫專案**，提供資料庫 schema、後端 API、語料匯入、音檔管理、審核工作台與資料治理規格。
...
```bash
mysql < database/migrations/0001_core_schema.sql
mysql < database/migrations/0002_indexes_views.sql
mysql < database/seeds/001_sources.sql
mysql < database/seeds/002_communities.sql
mysql < database/seeds/003_verified_facts.sql
mysql < database/seeds/004_rituals.sql
mysql < database/seeds/005_preview_vocabulary_audio.sql
```
...
- 祭儀公開摘要與敏感內容審核
- 卑南語四方言語音語料庫
- FormosanBank/ePark 全量語料匯入管線
- G2P / IPA / TTS metadata 管理
- 音檔 URL 與鏡像佇列
- 後台審核與資料版本
- 搜尋索引與公開 view
- RBAC 權限 seed
...
目前直接內嵌 preview 語音資料為 **80 筆**。完整千筆級資料不直接宣稱已內嵌，必須執行：
...
本專案保存來源與授權 metadata，但不代表所有資料可商用或可公開。正式上線前，需依 `data/database/source_license_registry.json` 與 `data/database/content_status_workflow.json` 進行審核。
...
## v14 Performance / Security / Search Upgrade
...
```bash
APPLY_MIGRATIONS=true ./deploy/upgrade-v14.sh
```

v14 keeps `preview_subset = 80` separate from `full_corpus >= 1000`, and adds safer retry/quality-gate infrastructure for the full corpus import.
...
## v19 Frontend AI Composer + Secure Knowledge Vault

v19 changes article creation so the **frontend calls the AI provider**. The backend knowledge service does not generate article text. It provides source packets, verified claims, duplicate checks, sensitivity checks, review queue, and publish checks.

New verified-source additions:

- v19 new source-grounded claims: 49
- merged source-grounded claims: 76
- new public cards/search docs: 49 / 49
- backend draft-plan endpoint is deprecated; use frontend composer flow.

Security additions include HMAC nonce design, row visibility policy, source-claim read scopes, audit logs, AI draft validation findings, and content security findings.
