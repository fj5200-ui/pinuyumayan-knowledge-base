...
export type BlockedRelationFinding = {
  blockedTerm: string;
  action: 'block' | 'manual_review' | 'remove_tag';
  messageZh: string;
};
...
export function validateBlockedRelations(input: { title?: string; body?: string; sourceIds?: string[]; tags?: string[] }): { ok: boolean; findings: BlockedRelationFinding[] } {
  ...
  const findings = forbiddenRelations.blocked_terms
    ...
    .map((term: string) => ({ blockedTerm: term, action: 'block' as const, messageZh: '卑南文化遺址／卑南遺址不可作為卑南族文化知識來源。' }));
