...
async function withFallback<T>(action: () => Promise<T>, fallback: () => Promise<T>): Promise<T> {
  if (process.env.KNOWLEDGE_DATA_MODE === "static") return fallback();
  try { return await action(); } catch { return fallback(); }
}

export async function getBootstrapData(db: Db) {
...
export async function searchPublicKnowledge(db: Db, input: { q: string; limit?: number; entityTypes?: string[] }) {
  return withFallback(async () => {
...
export async function getRelatedKnowledge(db: Db, input: { entityType: string; entityId: string; limit?: number }) {
  return withFallback(async () => {
...
export async function getCommunityProfile(db: Db, communityKey: string) {
  return withFallback(async () => {
...
export async function listVocabularyAudio(db: Db, input: { dialectCode?: string; q?: string; categoryKey?: string; limit?: number }) {
  return withFallback(async () => {
...
export async function getKnowledgeBundle(db: Db, input: { includeVocabulary?: boolean; vocabularyLimit?: number }) {
...
export async function getDeltaSince(db: Db, input: { since: string; limit?: number }) {
