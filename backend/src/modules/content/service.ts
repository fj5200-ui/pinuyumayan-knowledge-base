...
async function tryDb<T>(action: () => Promise<T>, fallback: () => Promise<T>): Promise<T> {
  if (process.env.KNOWLEDGE_DATA_MODE === "static") return fallback();
  try { return await action(); } catch { return fallback(); }
}

export async function listContentCollections(db: Db, channel = "public") {
  return tryDb(async () => {
...
export async function getContentItemBySlug(db: Db, slug: string) {
  return tryDb(async () => {
    const result = await db.execute(sql`
