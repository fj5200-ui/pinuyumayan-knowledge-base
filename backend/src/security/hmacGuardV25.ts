...
export type HmacVerificationResult = {
  ok: boolean;
  failureCode?: "missing_header" | "timestamp_skew" | "invalid_signature" | "nonce_reused" | "hmac_disabled";
...
export function buildSignatureBase(req: Request, bodyString = "") {
...
export function verifyPinuyumayanHmacV25(req: Request, secret = process.env.PINUYUMAYAN_HMAC_SECRET ?? ""): HmacVerificationResult {
  if (process.env.PINUYUMAYAN_HMAC_ENABLED === "false") return { ok: true, failureCode: "hmac_disabled" };
  for (const header of REQUIRED_HEADERS) {
    if (!req.header(header)) return { ok: false, failureCode: "missing_header" };
  }
  const timestamp = Number(req.header("x-pinuyumayan-timestamp"));
  if (!Number.isFinite(timestamp)) return { ok: false, failureCode: "timestamp_skew" };
  const now = Date.now();
  if (Math.abs(now - timestamp) > 5 * 60 * 1000) return { ok: false, failureCode: "timestamp_skew" };
  if (!secret) return { ok: false, failureCode: "invalid_signature" };
