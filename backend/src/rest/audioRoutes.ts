...
export function registerAudioRoutes(app: Express) {
  ...
  app.get("/api/public/audio/head/:assetId", asyncRoute(async (req, res) => {
    ...
    if (!head) return fail(res, 404, "NOT_FOUND", "Audio asset not found");
  ...
  app.get("/api/public/audio/proxy/:assetId", asyncRoute(async (req, res) => {
    ...
    if (!asset) return fail(res, 404, "NOT_FOUND", "Audio asset not found");
    let parsed: URL;
    try { parsed = assertAllowedSource(asset); } catch (error) {
      return fail(res, (error as any).status ?? 403, "FORBIDDEN", (error as Error).message);
    }
