...
export function registerTtsSttMusicV48Routes(app: Express) {
  ...
  app.post("/api/internal/vps/v48/run-db-validation", async (req, res) => {
    ...
    if (!guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "blocked_validation_payload", blocked_terms: guard.blocked });
    try {
  ...
  app.post("/api/internal/speech-training/v48/bulk-review-action", async (req, res) => {
    ...
    const guard = guardText(body);
    if (!body.actor_id || !body.idempotency_key || !allowedBulkActions.has(action) || !selected.length || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_bulk_review_action", allowed_actions: [...allowedBulkActions], blocked_terms: guard.blocked });
    if (action === "reject_gate" && !body.decision_reason) return res.status(400).json({ ok: false, version: "v48", error: "reject_gate_requires_decision_reason" });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/speech-training/v48/review-history-event", async (req, res) => {
    ...
    const guard = guardText(body);
    if (missing.length || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_history_event", missing, blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/search/music/v48/aggregate-query-logs", async (req, res) => {
    const body = req.body ?? {};
    const guard = guardText(body);
    if (!guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "blocked_analytics_payload", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/search/music/v48/alert-notification", async (req, res) => {
    const body = req.body ?? {};
    const guard = guardText(body);
    if (!body.rule_id || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_alert_notification", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/authority-sources/v48/merge-decision", async (req, res) => {
    ...
    const guard = guardText(body);
    if (!body.merge_group_id || !body.reviewer_id || !allowedMergeActions.has(action) || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_merge_decision", allowed_decisions: [...allowedMergeActions], blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/speech-training/v48/export-governance-artifact", async (req, res) => {
    const body = req.body ?? {};
    const guard = guardText(body);
    if (!body.artifact_format || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_governance_export", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/site/v48/visual-audit-record", async (req, res) => {
    const body = req.body ?? {};
    const guard = guardText(body);
    if (!body.route_path || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_visual_audit", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/site/v48/sitemap-ping-record", async (req, res) => {
    const body = req.body ?? {};
    const guard = guardText(body);
    if (!body.sitemap_url || !body.ping_target || !guard.ok) return res.status(400).json({ ok: false, version: "v48", error: "invalid_sitemap_ping", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
