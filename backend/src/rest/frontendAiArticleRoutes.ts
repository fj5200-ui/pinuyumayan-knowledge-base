...
function requireInternalKey(req: Request, res: Response, next: () => void) {
  ...
  if (!expected) return res.status(500).json({ ok: false, error: { code: 'server_key_not_configured' } });
  if (req.header('x-pinuyumayan-main-site-key') !== expected) return res.status(401).json({ ok: false, error: { code: 'unauthorized' } });
  next();
}

export function registerFrontendAiArticleRoutes(app: Express) {
  ...
  app.post('/api/internal/ai-article/client-draft/submit-review', requireInternalKey, (req, res) => {
    ...
    if (!result.ok) return res.status(422).json({ ok: false, error: { code: 'draft_validation_failed', findings: result.findings } });
