...
export function registerTtsSttMusicV44Routes(app: Express) {
  ...
  app.get("/api/public/music/v44/metadata/:id", (req, res) => {
    ...
    if (!doc) return res.status(404).json({ ok: false, error: "music_metadata_not_found", id });
    res.json({ ok: true, version: "v44", metadata_only: true, no_lyrics: true, no_audio_download: true, item: doc });
  });
  app.get("/api/public/music/v44/search-page", (_req, res) => res.json(readJson("data/site/main_site_music_tts_pages_v44.json").routes.find((r: any) => r.path === "/music/search")));
