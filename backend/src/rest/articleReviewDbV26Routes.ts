...
function checkForbidden(input: any) {
  const text = JSON.stringify(input ?? {});
  return FORBIDDEN.filter(term => text.includes(term));
}

export function registerArticleReviewDbV26Routes(app: Express) {
  app.post("/api/admin/articles/v26/review-action", async (req, res) => {
    const { draftId, action, reason, publishAt, checks } = req.body ?? {};
    if (!draftId || !action) return fail(res, 400, "BAD_REQUEST", "draftId and action are required");
    if (!NEXT_STATE[action]) return fail(res, 400, "BAD_REQUEST", "Unsupported review action");
    ...
    try {
      ...
      if (!admin) return fail(res, 401, "ADMIN_UNAUTHORIZED", "Admin session is required");
      const nextState = NEXT_STATE[action];
      await execute(
        `INSERT INTO article_review_transactions_v26 (draft_id, action, next_state, reviewer_admin_user_id, reason, publish_at, checks_json)
         VALUES (?, ?, ?, ?, ?, ?, CAST(? AS JSON))`,
        [draftId, action, nextState, admin.id, reason ?? null, publishAt ?? null, JSON.stringify(checks ?? {})]
      );
      await execute(
        `INSERT INTO article_review_audit_v26 (draft_id, event_type, actor_admin_user_id, metadata_json, request_id)
         VALUES (?, ?, ?, CAST(? AS JSON), ?)`,
        [draftId, `review.${action}`, admin.id, JSON.stringify({ reason: reason ?? null, publishAt: publishAt ?? null, forbiddenTerms }), (req as any).requestId ?? null]
      );
      if (action === "schedule") {
        await execute(
          `INSERT INTO content_publish_queue (content_item_id, action, scheduled_at, status, metadata_json)
           VALUES (?, 'publish', ?, 'scheduled', CAST(? AS JSON))`,
          [draftId, publishAt ?? null, JSON.stringify({ source: "articleReviewDbV26", reviewerAdminUserId: admin.id })]
        ).catch(() => null);
      }
