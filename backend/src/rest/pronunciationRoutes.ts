...
export function registerPronunciationRoutes(app: Express) {
  app.get("/api/public/pronunciation/:entryId", asyncRoute(async (req, res) => {
    ...
    if (!result.ok) return fail(res, 404, "NOT_FOUND", "Pronunciation audio was not found for this entry");
    ok(res, result);
  }));
  ...
  app.get("/api/public/tts/pronounce", asyncRoute(async (req, res) => {
    ...
    if (!text && !entryId) return fail(res, 400, "BAD_REQUEST", "Provide text or entryId");
