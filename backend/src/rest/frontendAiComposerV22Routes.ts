...
export function registerFrontendAiComposerV22Routes(app: Express) {
  app.get('/api/public/ai-article/v22/provider-adapters', (_req: Request, res: Response) => {
    res.json({ ok: true, version: 'v22', providers: ['local_mock_adapter', 'openai_responses_adapter', 'kimi_chat_adapter'] });
  });

  app.get('/api/public/ai-article/v22/source-packets', (_req: Request, res: Response) => {
    res.json({ ok: true, version: 'v22', message: 'Load data/ai/frontend_source_packets_v22.json in production service.' });
  });

  app.post('/api/internal/ai-article/v22/client-draft/validate', (req: Request, res: Response) => {
  ...
  app.post('/api/internal/ai-article/v22/citation-check', (req: Request, res: Response) => {
  ...
  app.post('/api/internal/ai-article/v22/duplicate-cooldown-check', (_req: Request, res: Response) => {
    res.json({ ok: true, decision: 'requires_database_check', policy: 'same_claim_set_30_days + same_topic_10_days' });
  });

  app.get('/api/admin/ai-article/v22/review-dashboard', (_req: Request, res: Response) => {
