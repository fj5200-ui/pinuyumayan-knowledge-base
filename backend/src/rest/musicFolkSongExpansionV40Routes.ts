...
function readJson(rel: string) { const root = process.cwd().endsWith("backend") ? path.resolve(process.cwd(), "..") : process.cwd(); return JSON.parse(fs.readFileSync(path.join(root, rel), "utf8")); }
const blockedTerms = ["卑南文化遺址", "卑南遺址", "卑南考古遺址", "卑南文化公園", "卑南遺址公園", "Peinan Site", "Beinan Site", "Peinan Archaeological Site"];
export function registerMusicFolkSongExpansionV40Routes(app: Express) {
