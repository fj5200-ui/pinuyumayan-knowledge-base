...
export function registerVpsDatabaseV26Routes(app: Express) {
  ...
  app.post("/api/internal/vps-db/v26/backup-report", async (req, res) => {
    try {
      ...
      if (!backupKey || !instanceKey || !storageUri) return fail(res, 400, "BAD_REQUEST", "backupKey, instanceKey and storageUri are required");
      await execute(
        `INSERT INTO vps_database_backups_v26 (backup_key, instance_key, storage_uri, sha256, size_bytes, status, completed_at)
         VALUES (?, ?, ?, ?, ?, ?, IF(? IN ('completed','failed'), NOW(), NULL))
         ON DUPLICATE KEY UPDATE storage_uri=VALUES(storage_uri), sha256=VALUES(sha256), size_bytes=VALUES(size_bytes), status=VALUES(status), completed_at=VALUES(completed_at)`,
        [backupKey, instanceKey, storageUri, sha256 ?? null, sizeBytes ?? null, status ?? "completed", status ?? "completed"]
      );
