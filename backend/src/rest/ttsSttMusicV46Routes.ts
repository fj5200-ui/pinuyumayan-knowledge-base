...
export function registerTtsSttMusicV46Routes(app: Express) {
  ...
  app.post("/api/internal/speech-training/v46/assign-reviewer", async (req, res) => {
    ...
    if (missing.length || !guard.ok) return res.status(400).json({ ok: false, version: "v46", error: "invalid_assignment", missing, blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
        await conn.execute(
          "UPDATE speech_reviewer_queue_v46 SET assigned_reviewer_id=?, assigned_by=?, assigned_at=NOW(), due_at=DATE_ADD(NOW(), INTERVAL 72 HOUR), queue_status='assigned' WHERE queue_id=? AND asset_id=?",
          [body.reviewer_id, body.assigned_by, body.queue_id, body.asset_id]
        );
        await conn.execute(
          "INSERT IGNORE INTO speech_review_sla_events_v46 (event_id, queue_id, asset_id, event_type, actor_id, event_json) VALUES (?,?,?,?,?,CAST(? AS JSON))",
          [`sla-v46-${guard.request_hash.slice(0, 16)}`, body.queue_id, body.asset_id, "assigned", body.assigned_by, JSON.stringify(body)]
        );
  ...
  app.post("/api/internal/speech-training/v46/transactional-review-decision", async (req, res) => {
    ...
    const guard = guardText(body);
    if (missing.length || !allowedReviewDecisions.has(decision) || !guard.ok) return res.status(400).json({ ok: false, version: "v46", error: "invalid_review_decision", missing, allowed_decisions: [...allowedReviewDecisions], blocked_terms: guard.blocked });
    if (decision === "approve_gate" && !body.evidence_attachment_ids?.length) return res.status(400).json({ ok: false, version: "v46", error: "approve_requires_evidence_attachment_ids" });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
        const transactionId = `speech-review-tx-v46-${guard.request_hash.slice(0, 16)}`;
        await conn.execute(
          "INSERT INTO speech_review_transactions_v46 (transaction_id, queue_id, asset_id, reviewer_id, decision, decision_reason, request_hash, idempotency_key, audit_json, committed, public_release_allowed) VALUES (?,?,?,?,?,?,?,?,CAST(? AS JSON),TRUE,FALSE)",
          [transactionId, body.queue_id, body.asset_id, body.reviewer_id, decision, body.decision_reason, guard.request_hash, body.idempotency_key, JSON.stringify(body)]
        );
        ...
        await conn.execute(
          "INSERT IGNORE INTO speech_review_sla_events_v46 (event_id, queue_id, asset_id, event_type, actor_id, event_json) VALUES (?,?,?,?,?,CAST(? AS JSON))",
          [`decision-event-v46-${guard.request_hash.slice(0, 16)}`, body.queue_id, body.asset_id, decision, body.reviewer_id, JSON.stringify(body)]
        );
  ...
  app.post("/api/internal/speech-training/v46/transactional-alignment-import", async (req, res) => {
    ...
    const guard = guardText(body);
    if (missing.length || score < 0.85 || !guard.ok) return res.status(400).json({ ok: false, version: "v46", error: "invalid_alignment_import", missing, minimum_score: 0.85, blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
        const transactionId = `alignment-tx-v46-${guard.request_hash.slice(0, 16)}`;
        await conn.execute(
          "INSERT INTO speech_alignment_transactions_v46 (transaction_id, asset_id, reviewer_id, alignment_score, transcript_hash, request_hash, alignment_json, committed) VALUES (?,?,?,?,?,?,CAST(? AS JSON),TRUE)",
          [transactionId, body.asset_id, body.reviewer_id, score, sha(body.transcript_text), guard.request_hash, JSON.stringify(body.alignment_json ?? body)]
        );
        return { transaction_id: transactionId };
      });
  ...
  app.post("/api/internal/search/music/v46/query-log", async (req, res) => {
    ...
    if (!q) return res.status(400).json({ ok: false, version: "v46", error: "query_text_required" });
    const guard = guardText(body);
    if (!guard.ok) return res.status(400).json({ ok: false, version: "v46", error: "blocked_query_log", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
        const logId = `music-query-v46-${guard.request_hash.slice(0, 16)}`;
        await conn.execute(
          "INSERT IGNORE INTO music_search_query_logs_v46 (log_id, query_text, normalized_query, result_count, latency_ms, mode, locale, referrer_path, ip_hash, facets_json, suggestions_json) VALUES (?,?,?,?,?,?,?,?,?,CAST(? AS JSON),CAST(? AS JSON))",
          [logId, q, q.toLowerCase(), resultCount, Number(body.latency_ms ?? 0), body.mode ?? null, body.locale ?? "zh-Hant", body.referrer_path ?? null, body.ip_hash ?? null, JSON.stringify(body.facets ?? {}), JSON.stringify(body.suggestions ?? [])]
        );
        if (resultCount === 0) {
          await conn.execute(
            "INSERT IGNORE INTO music_search_zero_result_events_v46 (event_id, query_text, normalized_query, suggestions_json) VALUES (?,?,?,CAST(? AS JSON))",
            [`zero-v46-${guard.request_hash.slice(0, 16)}`, q, q.toLowerCase(), JSON.stringify(body.suggestions ?? [])]
          );
        }
  ...
  app.post("/api/internal/authority-sources/v46/retry-or-merge", async (req, res) => {
    ...
    const guard = guardText(body);
    if (!guard.ok || !["retry_fetch", "merge_candidates", "record_robots_tos_review"].includes(action)) return res.status(400).json({ ok: false, version: "v46", error: "invalid_authority_action", blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
  ...
  app.post("/api/internal/speech-training/v46/model-experiment-decision", async (req, res) => {
    ...
    const guard = guardText(body);
    if (missing.length || !allowedModelDecisions.has(decision) || !guard.ok) return res.status(400).json({ ok: false, version: "v46", error: "invalid_model_experiment_decision", missing, allowed_decisions: [...allowedModelDecisions], blocked_terms: guard.blocked });
    try {
      const tx = await withMysqlTransaction(async (conn) => {
