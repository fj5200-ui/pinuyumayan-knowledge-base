...
function reportAccepted(req: Request, res: Response, kind: string) {
  const payload = req.body ?? {};
  res.json({ ok: true, kind, receivedAt: new Date().toISOString(), payloadHash: sha256(payload), status: payload.status ?? "received" });
}

export function registerProductionCutoverV30Routes(app: Express) {
  ...
  app.get("/api/ops/cutover/v30/readiness", (_req, res) => {
    const checklist: any = readJson("data/deployment/production_cutover_checklist_v30.json", { phases: [] });
