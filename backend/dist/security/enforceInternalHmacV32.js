...
export function enforceInternalHmacV32() {
    return (req, res, next) => {
        if (process.env.PINUYUMAYAN_HMAC_ENABLED !== "true")
            return next();
        ...
        const failures = [];
        if (!secret)
            failures.push("missing_server_hmac_secret");
        if (expectedApiKey && apiKey !== expectedApiKey)
            failures.push("invalid_api_key");
        if (!timestamp)
            failures.push("missing_timestamp");
        if (!nonce)
            failures.push("missing_nonce");
        if (!signature)
            failures.push("missing_signature");
        const now = Date.now();
        const ts = timestamp ? Number(timestamp) : 0;
        if (timestamp && (!Number.isFinite(ts) || Math.abs(now - ts) > 300_000))
            failures.push("timestamp_skew");
        if (nonce) {
            const nonceKey = `${apiKey ?? "unknown"}:${nonce}`;
            const existing = seenNonces.get(nonceKey);
            if (existing && existing > now)
                failures.push("replayed_nonce");
            seenNonces.set(nonceKey, now + 600_000);
            for (const [k, expiry] of Array.from(seenNonces.entries()))
                if (expiry < now)
                    seenNonces.delete(k);
        }
