...
export function sha256HexV26(input) {
    return crypto.createHash("sha256").update(input).digest("hex");
}
export function signatureBaseV26(req) {
...
async function auditFailure(req, failureCode) {
    try {
        await execute(`INSERT INTO internal_hmac_failures_v26 (client_id, failure_code, request_path, request_method, ip_hash, user_agent_hash, request_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [req.header("x-pinuyumayan-client-id") ?? null, failureCode, req.originalUrl, req.method, sha256HexV26(req.ip ?? ""), sha256HexV26(req.header("user-agent") ?? ""), req.requestId ?? null]);
    }
    catch { /* audit must never crash the request */ }
}
export async function verifyInternalHmacV26(req) {
    ...
    if (!secret)
        return { ok: false, code: "missing_server_secret" };
    ...
    await execute(`INSERT INTO internal_api_nonces_v26 (client_id, nonce, request_timestamp_ms, request_path, request_method, expires_at)
     VALUES (?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE))`, [clientId, nonce, timestamp, req.originalUrl, req.method]);
    return { ok: true, clientId };
}
export function requireInternalHmacV26(req, res, next) {
    verifyInternalHmacV26(req)
        .then(result => {
        if (result.ok)
            return next();
        auditFailure(req, String(result.code ?? "unknown")).finally(() => {
            const code = String(result.code ?? "invalid_hmac");
