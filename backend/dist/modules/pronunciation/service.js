...
export async function loadPronunciationCatalog() {
    if (catalogCache && process.env.PINUYUMAYAN_DISABLE_STATIC_JSON_CACHE !== "true")
        return catalogCache;
    const payload = await readJson("data/audio/puyuma_pronunciation_catalog_v16.json");
    catalogCache = payload.entries ?? [];
    return catalogCache;
}
export async function loadVoiceModelRegistry() {
    if (modelCache && process.env.PINUYUMAYAN_DISABLE_STATIC_JSON_CACHE !== "true")
        return modelCache;
    const payload = await readJson("data/audio/puyuma_tts_voice_model_registry_v16.json");
...
export async function getPronunciationForEntry(entryId) {
...
export async function findPronunciation(input) {
    const rows = await loadPronunciationCatalog();
    let candidates = rows;
    if (input.dialectCode)
        candidates = candidates.filter((row) => row.language?.dialect_code === input.dialectCode);
    if (input.entryId) {
        const exact = candidates.find((row) => row.entry_id === input.entryId || row.id === input.entryId);
        if (exact)
            return { ok: true, matchType: "entry_id", item: exact };
    }
    const q = normalizeText(String(input.text ?? ""));
    if (q) {
        const exactText = candidates.find((row) => row.text?.normalized_for_match === q);
        if (exactText)
            return { ok: true, matchType: "exact_text", item: exactText };
        ...
        if (fuzzy)
            return { ok: true, matchType: "partial_text", item: fuzzy };
    }
    return { ok: false, matchType: "none", item: null };
}
export async function requestTtsLikeSourceAudio(input) {
