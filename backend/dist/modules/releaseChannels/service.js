...
export function listReleaseChannels(clientKey) {
    if (!clientKey)
        return RELEASE_CHANNELS.filter((channel) => channel.isPublic);
    return RELEASE_CHANNELS.filter((channel) => channel.allowedClients.includes(clientKey));
}
export function canClientReadChannel(clientKey, channel) {
    return RELEASE_CHANNELS.some((item) => item.channel === channel && item.allowedClients.includes(clientKey));
}
export function assertPublicSafeChannel(channel) {
