...
export class FrontendAiArticleService {
    ...
    resolveSourcePack(claimIds) {
        const allow = new Set(['public', 'public_summary_only']);
        const claims = this.claims.filter((claim) => claimIds.includes(claim.claim_id) &&
    ...
    validateClientDraft(input) {
        const findings = [];
        if (!input.title_zh?.trim())
            findings.push({ severity: 'blocker', key: 'missing_title', message_zh: '缺少文章標題。' });
        if (!input.slug?.trim())
            findings.push({ severity: 'blocker', key: 'missing_slug', message_zh: '缺少 slug。' });
        if (!input.claim_ids?.length)
            findings.push({ severity: 'blocker', key: 'missing_claim_ids', message_zh: '文章必須引用至少一個 source claim。' });
        if (!input.source_ids?.length)
            findings.push({ severity: 'blocker', key: 'missing_source_ids', message_zh: '文章必須保留來源 ID。' });
        for (const pattern of BLOCKED_PATTERNS) {
            if (pattern.test(input.body_markdown || ''))
                findings.push({ severity: 'blocker', key: 'sensitive_or_unsourced_generation', message_zh: '草稿疑似包含未授權史料、祭儀操作或敏感內容。' });
        }
