...
function accepted(req, res, kind) { const payload = req.body ?? {}; res.json({ ok: true, version: "v34", kind, payloadHash: sha256(payload), status: payload.status ?? "received", receivedAt: new Date().toISOString() }); }
export function registerExpandedTrueKnowledgeV34Routes(app) {
