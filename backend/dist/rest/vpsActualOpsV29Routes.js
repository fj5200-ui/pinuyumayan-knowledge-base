...
function runKey(prefix) {
    return `${prefix}-${new Date().toISOString().replace(/[:.]/g, '-')}`;
}
export function registerVpsActualOpsV29Routes(app) {
    app.get('/api/ops/vps/v29/readiness', async (_req, res) => {
    ...
    app.post('/api/internal/vps/v29/full-corpus-run/start', async (req, res) => {
        const key = req.body?.runKey ?? runKey('full-corpus-v29');
        const minEntries = Number(req.body?.minEntries ?? 1000);
        if (isDbConfigured()) {
            await execute(`INSERT INTO full_corpus_execution_runs_v29 (run_key, environment, database_target, min_entries, status, operator, started_at)
         VALUES (?, ?, ?, ?, 'running', ?, NOW())
         ON DUPLICATE KEY UPDATE status='running', started_at=NOW()`, [key, req.body?.environment ?? 'staging', req.body?.databaseTarget ?? 'vps-db', minEntries, req.body?.operator ?? null]);
        }
    ...
    app.post('/api/internal/vps/v29/full-corpus-run/report', async (req, res) => {
        const body = req.body ?? {};
        ...
        if (isDbConfigured()) {
            await execute(`INSERT INTO full_corpus_execution_runs_v29
    ...
    app.post('/api/internal/search/v29/populate-db', async (req, res) => {
    ...
    app.get('/api/ops/search/v29/population-runs', async (_req, res) => {
    ...
    app.get('/api/ops/fallback/v29/route-coverage', async (_req, res) => {
        ...
        const uncovered = rows.filter((r) => !r.production_covered && r.fallback_policy === 'db_required').length;
        res.json({ ok: true, data: { routeCoverage: rows, uncoveredDbRequiredRoutes: uncovered } });
    });
    app.post('/api/internal/fallback/v29/coverage-report', async (req, res) => {
    ...
    app.get('/api/ops/vps-db/v29/backup-restore-checksum', async (_req, res) => {
    ...
    app.post('/api/internal/vps-db/v29/backup-restore-checksum', async (req, res) => {
        const body = req.body ?? {};
    ...
    app.get('/api/admin/source-candidates/v29/reviews', async (_req, res) => {
    ...
    app.get('/api/ops/next-upgrade-plan/v30', (_req, res) => {
