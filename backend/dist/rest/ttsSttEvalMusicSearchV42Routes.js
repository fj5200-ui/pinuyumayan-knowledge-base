...
export function registerTtsSttEvalMusicSearchV42Routes(app) {
    ...
    app.get("/api/public/search/music", (req, res) => {
        ...
        const hits = q ? docs.filter((d) => JSON.stringify(d).toLowerCase().includes(q.toLowerCase())).slice(0, Number(req.query.limit ?? 20)) : docs.slice(0, Number(req.query.limit ?? 20));
