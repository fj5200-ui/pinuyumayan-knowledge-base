...
function toBooleanFulltextQuery(query) {
    return query
        .split(/[\s,，、]+/)
...
function expandMusicQueryV45(query) {
    ...
    for (const item of synonyms.items ?? []) {
        ...
        if (canonical && (lower.includes(canonical.toLowerCase()) || aliases.some((alias) => lower.includes(alias.toLowerCase())))) {
            terms.push(canonical, ...aliases.slice(0, 4));
        }
    }
    return Array.from(new Set(terms.filter(Boolean))).join(" ");
}
function facetCountsV45(hits) {
...
function zeroResultSuggestionsV45(_query) {
    const { quality } = musicSearchQualityV45();
    return quality?.zero_result_suggestions ?? ["卑南族 歌謠", "南王 卑南族", "知本 卑南族 音樂"];
}
async function writeMusicSearchQueryLogV47(input) {
    if (!pool)
        return { query_log_mode: "database_unavailable", committed: false };
    const requestHash = sha(input);
    const logId = `music-query-v47-${requestHash.slice(0, 16)}`;
    try {
        await pool.execute("INSERT IGNORE INTO music_search_query_logs_v47 (log_id, query_text, normalized_query, result_count, latency_ms, mode, locale, referrer_path, facet_counts_json, suggestions_json, source_route) VALUES (?,?,?,?,?,?,?,?,CAST(? AS JSON),CAST(? AS JSON),?)", [logId, input.query, input.query.toLowerCase(), input.count, input.latency_ms, input.mode, "zh-Hant", input.referrer_path ?? null, JSON.stringify(input.facet_counts ?? {}), JSON.stringify(input.suggestions ?? []), "/api/public/search/music/v43"]);
...
async function queryMusicFulltext(req) {
    if (!pool)
        return null;
...
function queryMusicStatic(req) {
    const q = String(req.query.q ?? "").trim();
    const docs = readJson("data/search/public_search_documents_v43.json").documents ?? [];
    const limit = normalizeLimit(req.query.limit);
    const hits = q
        ? docs.filter((d) => JSON.stringify(d).toLowerCase().includes(q.toLowerCase())).slice(0, limit)
        : docs.slice(0, limit);
    return hits;
}
export function registerTtsSttLiveMusicV43Routes(app) {
    ...
    app.get("/api/public/search/music/v43", async (req, res) => {
        ...
        if (blocked.length)
            return res.status(400).json({ ok: false, error: "forbidden_relation_query", blocked_terms: blocked });
        try {
