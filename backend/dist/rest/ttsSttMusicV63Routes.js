...
export function registerTtsSttMusicV63Routes(app) {
    ...
    app.post("/api/internal/vps/v63/append-final-ledger-entry", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.ledger_id ?? id("final-ledger-v63", body);
            const tx = await writeEvent("release_final_ledger_entries_v63", "ledger_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
        }
    });
    app.post("/api/internal/vps/v63/record-real-dns-cloudflare-snapshot", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.snapshot_id ?? id("dns-cloudflare-v63", body);
            const tx = await writeEvent("release_real_dns_cloudflare_snapshots_v63", "snapshot_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/vps/v63/record-real-restore-rollback-evidence", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.evidence_id ?? id("restore-rollback-v63", body);
            const tx = await writeEvent("release_real_restore_rollback_evidence_v63", "evidence_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/vps/v63/record-real-observation-window", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.window_id ?? id("observation-window-v63", body);
            const tx = await writeEvent("release_real_observation_windows_v63", "window_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/vps/v63/record-final-audit-sample", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.sample_id ?? id("audit-sample-v63", body);
            const tx = await writeEvent("release_final_audit_samples_v63", "sample_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/vps/v63/mark-release-final-status", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.status_id ?? id("release-status-v63", body);
            const tx = await writeEvent("release_certificate_final_statuses_v63", "status_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/speech-training/v63/create-real-output-batch", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.batch_id ?? id("real-output-batch-v63", body);
            const tx = await writeEvent("legal_speech_real_output_batches_v63", "batch_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/speech-training/v63/export-real-train-dev-test", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.export_id ?? id("real-export-v63", body);
            const tx = await writeEvent("legal_speech_train_dev_test_exports_v63", "export_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/speech-training/v63/lock-dataset-hash", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.lock_id ?? id("dataset-hash-lock-v63", body);
            const tx = await writeEvent("legal_speech_dataset_hash_locks_v63", "lock_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/speech-training/v63/release-model-card", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.model_card_id ?? id("model-card-release-v63", body);
            const tx = await writeEvent("legal_speech_model_card_releases_v63", "model_card_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/speech-training/v63/write-blocked-report", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.report_id ?? id("blocked-report-v63", body);
            const tx = await writeEvent("legal_speech_blocked_reports_v63", "report_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/search/music/v63/write-weekly-operations-report", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.report_id ?? id("search-weekly-ops-v63", body);
            const tx = await writeEvent("search_weekly_operations_reports_v63", "report_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/search/music/v63/create-sla-task", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.task_id ?? id("search-sla-task-v63", body);
            const tx = await writeEvent("search_sla_tasks_v63", "task_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/search/music/v63/record-rollback-drill-result", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.drill_id ?? id("search-rollback-drill-v63", body);
            const tx = await writeEvent("search_rollback_drill_results_v63", "drill_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/search/music/v63/record-quality-regression-result", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.test_id ?? id("search-regression-v63", body);
            const tx = await writeEvent("search_quality_regression_results_v63", "test_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/authority/v63/record-public-expansion", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.record_id ?? id("authority-public-expansion-v63", body);
            const tx = await writeEvent("authority_public_expansion_records_v63", "record_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/authority/v63/seal-citation-screenshot", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.evidence_id ?? id("citation-screenshot-v63", body);
            const tx = await writeEvent("authority_public_citation_screenshots_v63", "evidence_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/authority/v63/seal-sitemap-og-evidence", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.evidence_id ?? id("sitemap-og-v63", body);
            const tx = await writeEvent("authority_public_sitemap_og_evidence_v63", "evidence_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/authority/v63/seal-takedown-rehearsal", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.evidence_id ?? id("takedown-rehearsal-v63", body);
            const tx = await writeEvent("authority_public_takedown_rehearsals_v63", "evidence_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/authority/v63/seal-source-drift-snapshot", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.snapshot_id ?? id("source-drift-v63", body);
            const tx = await writeEvent("authority_public_source_drift_snapshots_v63", "snapshot_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/governance/v63/record-live-download-alert", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.alert_id ?? id("governance-live-alert-v63", body);
            const tx = await writeEvent("governance_download_live_alerts_v63", "alert_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/governance/v63/ack-live-download-alert", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.ack_id ?? id("governance-alert-ack-v63", body);
            const tx = await writeEvent("governance_download_live_alert_acks_v63", "ack_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/governance/v63/close-live-download-alert", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.closure_id ?? id("governance-alert-close-v63", body);
            const tx = await writeEvent("governance_download_live_alert_closures_v63", "closure_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/governance/v63/record-rbac-watermark-test", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.test_id ?? id("governance-rbac-watermark-v63", body);
            const tx = await writeEvent("governance_rbac_watermark_tests_v63", "test_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/governance/v63/signoff-audit-export", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.signoff_id ?? id("governance-audit-signoff-v63", body);
            const tx = await writeEvent("governance_audit_export_signoffs_v63", "signoff_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/ops/v63/record-live-notification-send", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.send_id ?? id("ops-live-send-v63", body);
            const tx = await writeEvent("operations_live_notification_sends_v63", "send_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/ops/v63/ack-live-notification", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.ack_id ?? id("ops-live-ack-v63", body);
            const tx = await writeEvent("operations_live_notification_acks_v63", "ack_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/ops/v63/escalate-live-notification", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.escalation_id ?? id("ops-live-escalate-v63", body);
            const tx = await writeEvent("operations_live_notification_escalations_v63", "escalation_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/ops/v63/close-live-notification", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.closure_id ?? id("ops-live-close-v63", body);
            const tx = await writeEvent("operations_live_notification_closures_v63", "closure_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/ops/v63/create-weekly-review-task", async (req, res) => {
        ...
        if (!check.ok)
            return res.status(400).json({ ok: false, version: "v63", error: "invalid_request", missing: check.missing, blocked_terms: check.blocked });
        try {
            const eventId = body.task_id ?? id("ops-weekly-review-v63", body);
            const tx = await writeEvent("operations_weekly_review_tasks_v63", "task_id", eventId, body);
            res.json({ ok: true, accepted: true, version: "v63", ...tx, real_vps_completed: false, dataset_export_allowed: false, public_audio_allowed: false });
        }
        catch (error) {
            res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    ...
    app.post("/api/internal/vps/v63/append-final-certificate-ledger-entry", async (req, res) => { const body = req.body ?? {}; try {
        const tx = await writeEvent("release_final_ledger_entries_v63", "ledger_entry_id", body.ledger_entry_id ?? id("final-certificate-v63", body), body);
        res.json({ ok: true, version: "v63", ...tx, real_vps_completed: false });
    }
    catch (error) {
        res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    } });
    app.post("/api/internal/speech-training/v63/create-immutable-release-batch", async (req, res) => { const body = req.body ?? {}; try {
        const tx = await writeEvent("legal_speech_immutable_release_batches_v63", "batch_id", body.batch_id ?? id("immutable-release-v63", body), body);
        res.json({ ok: true, version: "v63", ...tx, dataset_export_allowed: false });
    }
    catch (error) {
        res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    } });
    app.post("/api/internal/search/music/v63/write-weekly-improvement-report", async (req, res) => { const body = req.body ?? {}; try {
        const tx = await writeEvent("search_weekly_improvement_reports_v63", "report_id", body.report_id ?? id("weekly-improvement-v63", body), body);
        res.json({ ok: true, version: "v63", ...tx });
    }
    catch (error) {
        res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    } });
    app.post("/api/internal/authority/v63/record-public-evidence", async (req, res) => { const body = req.body ?? {}; try {
        const tx = await writeEvent("authority_public_evidence_records_v63", "record_id", body.record_id ?? id("public-evidence-v63", body), body);
        res.json({ ok: true, version: "v63", ...tx, public_audio_allowed: false });
    }
    catch (error) {
        res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    } });
    app.post("/api/internal/governance/v63/record-delivery-download-alert", async (req, res) => { const body = req.body ?? {}; try {
        const tx = await writeEvent("governance_delivery_download_alerts_v63", "alert_id", body.alert_id ?? id("delivery-alert-v63", body), body);
        res.json({ ok: true, version: "v63", ...tx });
    }
    catch (error) {
        res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
    } });
    app.post("/api/internal/ops/v63/record-audit-notification-send", async (req, res) => { const body = req.body ?? {}; try {
        const tx = await writeEvent("operations_audit_notification_sends_v63", "send_id", body.send_id ?? id("audit-notification-v63", body), body);
        res.json({ ok: true, version: "v63", ...tx });
    }
    catch (error) {
        res.status(500).json({ ok: false, version: "v63", error: "transaction_failed_rolled_back", message: error instanceof Error ? error.message : String(error) });
