...
export function registerTtsSttMusicV45Routes(app) {
    ...
    app.post("/api/internal/speech-training/v45/review-decision", (req, res) => {
        ...
        if (approveNeedsEvidence)
            return res.status(400).json({ ok: false, accepted: false, version: "v45", error: "approve_requires_evidence_url" });
    ...
    app.get("/api/public/music/v45/seo/:id", (req, res) => {
        ...
        if (!doc)
            return res.status(404).json({ ok: false, error: "music_seo_metadata_not_found", id });
