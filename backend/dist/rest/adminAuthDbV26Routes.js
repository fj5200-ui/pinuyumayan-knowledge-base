...
export function registerAdminAuthDbV26Routes(app) {
    app.post("/api/admin/auth/v26/login", async (req, res) => {
        const { email, password } = req.body ?? {};
        if (!email || !password)
            return fail(res, 400, "BAD_REQUEST", "email and password are required");
        try {
            const rows = await queryRows(`SELECT * FROM admin_users WHERE email = ? LIMIT 1`, [String(email).toLowerCase()]);
            const user = rows[0];
            const valid = user && user.status !== "disabled" && verifyPassword(String(password), String(user.password_hash));
            await execute(`INSERT INTO admin_login_attempts (email, success, failure_reason, ip_hash, user_agent_hash, request_id) VALUES (?, ?, ?, ?, ?, ?)`, [String(email).toLowerCase(), Boolean(valid), valid ? null : "invalid_credentials", hashOptional(req.ip), hashOptional(req.header("user-agent") ?? undefined), req.requestId ?? null]);
            if (!valid)
                return fail(res, 401, "ADMIN_UNAUTHORIZED", "Invalid admin credentials");
    ...
    app.get("/api/admin/auth/v26/me", async (req, res) => {
        try {
            ...
            if (!admin)
                return fail(res, 401, "ADMIN_UNAUTHORIZED", "Admin session is missing or expired");
            const roles = await queryRows(`SELECT role_key FROM admin_user_roles WHERE admin_user_id = ?`, [admin.id]);
    ...
    app.post("/api/admin/auth/v26/change-password", async (req, res) => {
        ...
        try {
            ...
            if (!admin)
                return fail(res, 401, "ADMIN_UNAUTHORIZED", "Admin session is missing or expired");
            const rows = await queryRows(`SELECT password_hash FROM admin_users WHERE id = ? LIMIT 1`, [admin.id]);
            if (!rows[0] || !verifyPassword(String(currentPassword ?? ""), String(rows[0].password_hash)))
                return fail(res, 401, "ADMIN_UNAUTHORIZED", "Current password is invalid");
    ...
    app.post("/api/admin/auth/v26/logout", async (req, res) => {
        try {
            ...
            if (admin)
                await auditAdmin(Number(admin.id), "admin.logout", { targetType: "admin_user", targetId: String(admin.id), revoked }, req);
    ...
    app.get("/api/admin/auth/v26/bootstrap-status", async (_req, res) => {
        try {
            const rows = await queryRows(`SELECT COUNT(*) AS count FROM admin_users WHERE is_super_admin = TRUE`);
