...
export function registerTtsSttMusicV49Routes(app) {
    ...
    app.post("/api/internal/vps/v49/record-release-validation", async (req, res) => {
        ...
        if (!body.run_id || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_release_validation_payload", blocked_terms: guard.blocked });
        try {
    ...
    app.post("/api/internal/vps/v49/record-backup-restore-drill", async (req, res) => {
        const body = req.body ?? {};
        const guard = guardText(body);
        if (!body.actor_id || !body.backup_id || !body.restore_target || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_backup_restore_drill", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/speech-training/v49/workbench-action", async (req, res) => {
        ...
        const guard = guardText(body);
        if (!body.actor_id || !body.queue_id || !body.idempotency_key || !allowedWorkbenchActions.has(action) || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_workbench_action", allowed_actions: [...allowedWorkbenchActions], blocked_terms: guard.blocked });
        if (action === "approve_gate" && body.evidence_complete !== true)
            return res.status(400).json({ ok: false, version: "v49", error: "approve_gate_requires_evidence_complete" });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/speech-training/v49/attachment-scan-status", async (req, res) => {
        const body = req.body ?? {};
        const guard = guardText(body);
        if (!body.actor_id || !body.queue_id || !body.attachment_id || !body.scan_status || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_attachment_scan_status", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/speech-training/v49/batch-progress", async (req, res) => {
        const body = req.body ?? {};
        const guard = guardText(body);
        if (!body.actor_id || !body.batch_id || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_batch_progress", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/search/music/v49/apply-synonym-suggestion", async (req, res) => {
        const body = req.body ?? {};
        const guard = guardText(body);
        if (!body.suggestion_id || !body.actor_id || body.manual_review_passed !== true || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "synonym_apply_requires_manual_review", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/search/music/v49/record-regression-run", async (req, res) => {
        ...
        const guard = guardText(body);
        if (!body.run_id || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_regression_run", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/authority-sources/v49/approve-metadata-only", async (req, res) => {
        const body = req.body ?? {};
        const guard = guardText(body);
        if (!body.actor_id || !body.candidate_id || body.citation_score_passed !== true || body.rights_review_passed !== true || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "metadata_only_approval_requires_citation_and_rights_review", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/authority-sources/v49/source-change-event", async (req, res) => {
        ...
        if (!body.source_id || !body.event_type || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_source_change_event", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/speech-training/v49/export-model-governance", async (req, res) => {
        ...
        if (!body.actor_id || !body.export_format || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_model_governance_export", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/site/v49/record-performance-validation", async (req, res) => {
        ...
        if (!body.route_path || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_performance_validation", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/site/v49/record-og-screenshot-validation", async (req, res) => {
        ...
        if (!body.route_path || !body.status || !guard.ok)
            return res.status(400).json({ ok: false, version: "v49", error: "invalid_og_screenshot_validation", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
