...
export function registerArticleReviewV25Routes(app) {
    ...
    app.post("/api/internal/articles/v25/publish-seo-check", (req, res) => {
        ...
        if (!hmac.ok)
            return res.status(401).json({ ok: false, error: { code: hmac.failureCode, message: "Invalid internal API signature" } });
