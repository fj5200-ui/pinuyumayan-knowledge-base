...
export function registerVpsStagingV27Routes(app) {
    ...
    app.post("/api/internal/corpus/v27/acceptance-report", requireInternalHmacV26, async (req, res) => {
        try {
            ...
            await execute(`INSERT INTO full_corpus_acceptance_metrics_v27
    ...
    app.get("/api/ops/search/v27/index-status", async (_req, res) => {
        try {
            const counts = await queryRows(`SELECT release_channel, COUNT(*) AS count FROM search_index_documents_v27 GROUP BY release_channel`);
    ...
    app.post("/api/internal/vps-db/v27/restore-drill-report", requireInternalHmacV26, async (req, res) => {
        try {
            ...
            if (!drillKey || !backupKey || !restoreTarget)
                return fail(res, 400, "BAD_REQUEST", "drillKey, backupKey and restoreTarget are required");
            await execute(`INSERT INTO vps_backup_restore_drills_v27 (drill_key, backup_key, restore_target, status, row_count_checks_json, checksum_checks_json, restore_started_at, restore_finished_at, verified_at, operator_label, notes)
         VALUES (?, ?, ?, ?, CAST(? AS JSON), CAST(? AS JSON), NOW(), IF(? IN ('restored','verified','failed'), NOW(), NULL), IF(?='verified', NOW(), NULL), ?, ?)
         ON DUPLICATE KEY UPDATE status=VALUES(status), row_count_checks_json=VALUES(row_count_checks_json), checksum_checks_json=VALUES(checksum_checks_json), restore_finished_at=VALUES(restore_finished_at), verified_at=VALUES(verified_at), operator_label=VALUES(operator_label), notes=VALUES(notes)`, [drillKey, backupKey, restoreTarget, status ?? "started", JSON.stringify(rowCountChecks ?? {}), JSON.stringify(checksumChecks ?? {}), status ?? "started", status ?? "started", operatorLabel ?? null, notes ?? null]);
