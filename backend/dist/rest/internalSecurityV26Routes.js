...
export function registerInternalSecurityV26Routes(app) {
    app.post("/api/internal/security/v26/verify-hmac", async (req, res) => {
        try {
            const result = await verifyInternalHmacV26(req);
            return result.ok ? ok(res, { version: "v26", verified: true, clientId: result.clientId ?? null }) : fail(res, 401, "UNAUTHORIZED", `Invalid signature: ${result.code}`);
        }
        catch (error) {
