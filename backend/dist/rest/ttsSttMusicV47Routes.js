...
export function registerTtsSttMusicV47Routes(app) {
    ...
    app.post("/api/internal/speech-training/v47/upload-evidence-record", async (req, res) => {
        ...
        if (missing.length || !guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "invalid_evidence_record", missing, blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/speech-training/v47/review-action", async (req, res) => {
        ...
        const guard = guardText(body);
        if (missing.length || !allowedReviewActions.has(action) || !guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "invalid_review_action", missing, allowed_actions: [...allowedReviewActions], blocked_terms: guard.blocked });
        if (action === "approve_gate" && !body.evidence_attachment_ids?.length)
            return res.status(400).json({ ok: false, version: "v47", error: "approve_requires_evidence_attachment_ids", rollback_expected: true });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
                const actionId = `review-action-v47-${guard.request_hash.slice(0, 16)}`;
                let queueStatus = "assigned";
                if (action === "return_for_fix")
                    queueStatus = "returned_for_fix";
                if (action === "reject_gate")
                    queueStatus = "rejected_gate";
                if (action === "approve_gate")
                    queueStatus = "approved_gate_pending_all_gates";
                ...
                if (action === "import_alignment") {
                    ...
                    if (score < 0.85)
                        throw new Error("alignment_score_below_0_85");
    ...
    app.post("/api/internal/database/v47/run-transaction-test", async (req, res) => {
        ...
        const guard = guardText(body);
        if (!guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "blocked_test_payload", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/search/music/v47/query-log", async (req, res) => {
        ...
        if (!q)
            return res.status(400).json({ ok: false, version: "v47", error: "query_text_required" });
        const guard = guardText(body);
        if (!guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "blocked_query_log", blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/authority-sources/v47/live-fetch-run", async (req, res) => {
        ...
        const guard = guardText(body);
        if (!adapterId || !guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "invalid_authority_fetch_run", blocked_terms: guard.blocked });
    ...
    app.post("/api/internal/authority-sources/v47/candidate-review", async (req, res) => {
        ...
        const guard = guardText(body);
        if (!body.candidate_id || !allowedCandidateActions.has(action) || !guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "invalid_candidate_review", allowed_actions: [...allowedCandidateActions], blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
    ...
    app.post("/api/internal/speech-training/v47/model-governance-decision", async (req, res) => {
        ...
        if (missing.length || !allowedModelDecisions.has(decision) || !guard.ok)
            return res.status(400).json({ ok: false, version: "v47", error: "invalid_model_governance_decision", missing, allowed_decisions: [...allowedModelDecisions], blocked_terms: guard.blocked });
        try {
            const tx = await withMysqlTransaction(async (conn) => {
