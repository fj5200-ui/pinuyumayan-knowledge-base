...
export function runtimeEnvironmentV27() {
...
export function productionStaticFallbackAllowedV27() {
    if (runtimeEnvironmentV27() !== "production")
        return true;
    if (process.env.DISABLE_PRODUCTION_STATIC_FALLBACK === "false")
        return true;
    return false;
}
export async function recordDbFallbackEventV27(req, args) {
    try {
        const error = args.error instanceof Error ? args.error : undefined;
        await execute(`INSERT INTO db_fallback_events_v27 (route_path, route_group, environment, fallback_source, db_error_code, db_error_message, blocked, request_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [req.originalUrl, args.routeGroup, runtimeEnvironmentV27(), args.fallbackSource ?? null, error?.code ?? null, error?.message ?? null, args.blocked, req.requestId ?? null]);
    }
